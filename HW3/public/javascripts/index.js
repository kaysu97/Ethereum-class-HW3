    ...
    if (bankAddress != "") {
		$.get('/owner', {
          ...
          address: bankAddress
			...
			$('#getOwner').text('Owner帳戶: ' + result.owner)
...
transferCoinButton.on('click', async function () {
	...
	$.post('/transferCoin', {
		...
		value: parseInt(transferCoin.val(), 10)
	}, function (result) {
		if (result.events !== undefined) {
			log(result.events.TransferCoinEvent.returnValues, 'nccu coin轉帳成功')
...
transferETHButton.on('click', async function () {
   if (bankAddress == "") {
		return;
	}
	...
	$.post('/transferTo', {
		...
		value: parseInt(transferETH.val(), 10)
	}, function (result){
          if (result.events !== undefined) {
			log(result.events.PromiEvent.returnValue, '轉移owner成功')
